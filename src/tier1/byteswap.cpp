	...
	case FIELD_EMBEDDED:
		{
			...
			{
				SwapFieldsToTargetEndian( (byte*)pOutputBuffer + pEmbed->flatOffset[ TD_OFFSET_NORMAL ],
										(byte*)pData + pEmbed->flatOffset[ TD_OFFSET_NORMAL ],
										pField->td );
...
{	
	...
	for ( int i = 0; i < fieldCount; ++i )
	{
		...
		SwapFieldToTargetEndian( (BYTE*)pOutputBuffer + pField->flatOffset[ TD_OFFSET_NORMAL ],
								 (BYTE*)pBaseData + pField->flatOffset[ TD_OFFSET_NORMAL ],
								  pField );
	}
}
