...
void SplitScreenConVarRef::Init(const char* pName, bool bIgnoreMissing)
{
	...
	{
		m_Info[i].m_pConVar = g_pCVar ? g_pCVar->FindVar(pName) : &s_EmptyConVar;
		if (!m_Info[i].m_pConVar)
		{
			m_Info[i].m_pConVar = &s_EmptyConVar;
		}
		m_Info[i].m_pConVarState = static_cast<ConVar*>(m_Info[i].m_pConVar);
		if (!IsValid())
		{
			static bool bFirst = true;
			if (g_pCVar || bFirst)
